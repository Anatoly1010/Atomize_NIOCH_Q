...
class MainWindow(QMainWindow):
    ...
    def design_tab_2(self):
        ...
        double_boxes = [(QSpinBox, "Acq_number", "number_averages", self.acq_number, 1, 1e4, 1, 1, 0, ""),
                      ...
                      (QDoubleSpinBox, "Win_left", "cur_win_left", self.win_left, 0, 6400, 0, 0.4, 2, " ns"),
                      (QDoubleSpinBox, "Win_right", "cur_win_right", self.win_right, 0, 6400, 320, 0.4, 2, " ns"),
                      (QDoubleSpinBox, "Win_width", "win_width", self.win_width_func, 0.01, 6400, 100, 2, 2, " ns"),
    ...
    def design_tab_3(self):
        ...
        # ---- Boxes ----
        double_boxes = [(QSpinBox, "P_to_drop", "p_to_drop", self.p_to_drop_func, 0, 16000, 0, 1, 0, " pts"),
                      (QDoubleSpinBox, "Zero_order", "zero_order", self.zero_order_func, -0.1, 360.1, 0, 0.5, 4, " deg"),
                      (QDoubleSpinBox, "First_order", "first_order", self.first_order_func, -7200, 7200, 0, 0.001, 4, " deg/MHz"),
    ...
    def link_source_changed(self, index, suffix):
        ...
        try:
            ...
            for j in range(1, 10):
                ...
                # (which retunes the oscilloscope IQ demod) still propagates.
                if box_j.maximum() - box_j.minimum() < 1e-9:
                    continue
                shift = unit * f_j
    ...
    def _structure_sig(self, snap):
        ...
        sigma / type all re-arm live. A LASER on/off is caught via laser_flag,
        and the LASER rows' start / length and the laser source are included; a
    ...
    def update_pulse_type(self, index):
        ...
        # shape atomically). A LASER on/off instead changes laser_flag (the
        # laser row count), which is in the structure signature, so that case
        # auto-restarts.
        self.schedule_live_apply()
    ...
    def win_left(self):
        ...
        """
        self.cur_win_left = int(round(float(self.Win_left.value()) / self.time_per_point))
        record_points = getattr(self, 'preview_points', self.dig_points)
        if self.cur_win_left > record_points:
            self.cur_win_left = int( record_points )
            self.Win_left.setValue( round( self.cur_win_left * self.time_per_point, 2) )
    ...
    def win_right(self):
        self.cur_win_right = int(round(float(self.Win_right.value()) / self.time_per_point))
        record_points = getattr(self, 'preview_points', self.dig_points)
        if self.cur_win_right > record_points:
            self.cur_win_right = int( record_points )
            self.Win_right.setValue( round( self.cur_win_right * self.time_per_point, 2) )
    ...
    def open_file(self, filename):
        ...
        self.Win_left.setValue( round(float( lines[19].split(':  ')[1] ), 2) )
        self.Win_right.setValue( round(float( lines[20].split(':  ')[1] ), 2) )
    ...
    def decimat(self):
        ...
        if self.shift_together == 1:
            ...
            if self.posttrigger > self.dig_points - 16:
                ...
                self.posttrigger = ( ( self.dig_points - 16 ) // grid ) * grid
                self.Hor_offset.setValue( self.posttrigger )
        # Send PO/HO in an order that never creates a transient where the record
        # length is smaller than the posttrigger. The worker reconfigures the device
        # (stop + set + run/setup) on EACH message, so shrinking the window: lower the
        # offset first; growing it: raise the points first. (Harmless on the scope,
        # which tolerates offset > timebase; kept in parity with the NIOCH card.)
        try:
    ...
    def parse_message(self):
        ...
        if msg_type == 'Status':
            ...
            self.Win_left.setValue(round(left_ns, 2))
            self.Win_right.setValue(round(right_ns, 2))
        ...
        elif msg_type == 'DigitizerTiming':
            self.time_per_point, self.preview_points = data
            for box in (self.Win_left, self.Win_right, self.Win_width):
                box.blockSignals(True)
                box.setSingleStep(self.time_per_point)
                box.setMaximum(self.preview_points * self.time_per_point)
                box.blockSignals(False)
            self.Win_width.setMinimum(self.time_per_point)
            self.win_width_func()
            self.win_left()
            self.win_right()
...
class Worker():
    ...
    def dig_on(self, conn, dig_points, posttrigger, n_averages, win_left, win_right, rect1, rect2, rect3, rect4, rect5, rect6, rect7, n_wurst, rep_rate, mag_field, fft_flag, cur_phase, ch0_ampl, ch1_ampl, trig_delay, awg2, awg3, awg4, awg5, awg6, awg7, quad, zero_order, first_order, second_order, p_to_drop, b_sech, combo_cor, combo_synt, _reserved, rect8, rect9, awg8, awg9, laser_flag, laser_num, laser_qsw_delay, iq_corr, script_test=False ):
        ...
        try:
            ...
            pb.pulser_pulse(name='P0', channel='TRIGGER_AWG', start='0 ns', length='32 ns')
            ...
            if not script_test:
                conn.send(('DigitizerTiming', (t_res * 1000, WIN_ADC)))
            ...
            while self.command != 'exit':
                ...
                if self.command[0:2] == 'PO':
                    ...
                    # Spectrum AWG buffer (awg); Start deltas also move the PB_Micran
                    ...
                    # PB_Micran; a genuine collision is rejected and the running
                    # preview kept.
                    try:
                    ...
                    if snap is not None:
                        def _shift_start(orig_str, delta):
                            ...
                            return f"{self.round_to_closest(base + float(delta), 4)} ns"
                        ...
                        # Validate the proposed pulser sequence in a test-mode PB_Micran.
                        reject = None
                        saved_argv = sys.argv
                        try:
                            sys.argv = ['', 'test']
                            pbt = pb_pro.PB_Micran()
                        ...
                        if reject is not None:
                            ...
                            # pulse-geometry parameter -- it only sets the scope
                            # IQ demodulation frequency (iq_freq, consumed by oscilloscope_demodulate), applied per scan
                            ...
                            # Commit: Spectrum AWG waveform (awg) + PB_Micran gating (pb).
                ...
                if iq_cor == 1 or quad == 0:
                    data_x, data_y = t3104.oscilloscope_demodulate(data_x, data_y, iq_freq if iq_cor == 1 else 0,
                        zero_order if quad == 0 else 0, 0, 0)

                if script_test:
                    general.plot_1d('Dig', x_axis * 1e-6, ( data_x, data_y ),
                        xscale = 's', yscale = 'V', label = 'ch',
                        vline = (win_left * t_res * 1e-6, win_right * t_res * 1e-6)
                        )
                else:
        ...
        try:
                    ...
                    for j in range(POINTS):
                        ...
                        if (not script_test) or j == 0:
                            if iq_cor == 0:
                                ...
                                area_x, area_y = t3104.oscilloscope_demodulate(data[0], data[1], iq_freq, zp, 0, 0, integral = True)
                                if step != 1:
            ...
            if self.command == 'exit':
                ...
                if iq_cor == 0:
                    ...
                    data_x, data_y = t3104.oscilloscope_demodulate(data[0], data[1], iq_freq, zp, 0, 0, integral = True)
                    if step != 1:
            ...
            for cycle in range(cycle_count):
                ...
                if a is not None and not script_test:
                    if iq_cor == 0:
                        ...
                        rdx, rdy = t3104.oscilloscope_demodulate(data[0], data[1], iq_freq, zp, 0, 0, integral = True)
                        if step != 1:
                    ...
                    if save_each:
                        for idx in range(len(cycle_snapshots)):
                            ...
                            if iq_cor == 0:
                                ...
                                cdx, cdy = t3104.oscilloscope_demodulate(cdat[0], cdat[1], iq_freq, zp, 0, 0, integral = True)
        ...
        try:
            ...
            for n, lr in enumerate(laser_rects, start=1):
                ...
                if f'L{n}' in name_list:
                    laser_delta_start = f"{self.round_to_closest( nonlinear_diff[0] * rel_shift[rs_idx], 4 )} ns"
                    rs_idx += 1
                else:
