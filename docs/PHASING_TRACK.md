...
- 88 focused offscreen tests passed for this fork.
...
NIOCH_Q now sends Dig time coordinates and integration-window markers in
seconds in both live and preflight plotting branches. Internal scope timing
remains in microseconds; integration and FFT calculations are unchanged. Six
coordinate/marker checks cover both tools and three scope sample intervals.
...
Validation: 58 auto-control and Stop checks passed, including the real Qt
...
Validation: the full test suite passed (130 passed), including one-click
