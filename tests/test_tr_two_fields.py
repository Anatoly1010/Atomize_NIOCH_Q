...
def test_native_preflight_checks_both_ranges_without_io(monkeypatch, tmp_path, num_osc, reverse):
    ...
    magnet_module = importlib.import_module('atomize.device_modules.ITC_FC')
    set_field = magnet_module.ITC_FC.magnet_field
    current = [100]
    records = {}
    ...
    monkeypatch.setattr(magnet_module.ITC_FC, 'magnet_field', field)
    for name in ('Keysight_2000_Xseries',):
...
def test_real_gui_half_field_roundtrip(monkeypatch, tmp_path):
    monkeypatch.setenv('QT_QPA_PLATFORM', 'offscreen')
    from PyQt6.QtWidgets import QApplication
    from atomize.control_center.tr_control import MainWindow
    monkeypatch.chdir(Path(__file__).parents[1] / 'atomize/control_center')
    ...
    for box, value in zip(window.half_boxes, (110, 112, 0.5)):
        box.setValue(value)
    app.processEvents()
    assert all(not box.isHidden() for box in window.half_boxes)
    window.save_file(str(tmp_path / 'pair'))
    window.open_file(str(tmp_path / 'old.tr'))
    assert not window.enable_half.isChecked()
    window.open_file(str(tmp_path / 'pair.tr'))
    assert window.enable_half.isChecked()
    assert [box.value() for box in window.half_boxes] == [110, 112, 0.5]
    window.hide()
    window.deleteLater()
